...
#
# Tags look like `v<Immich Kiosk version>-<release>`:
#
# - if defaults/main.yml points at an Immich Kiosk version that has never been
#   released, the release counter restarts at 0 (`v0.43.0-0`)
# - otherwise the counter is incremented (`v0.43.0-1`), but only if something
...
version="$(sed -nE 's|^immich_kiosk_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Immich Kiosk version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `0.42.0`), but the tags
# do (`v0.42.0-0`). Stripping any `v` before prepending one keeps this correct
